...
class DependencyGraphBuilder<D>(
    ...
    fun build(checkReferences: Boolean = true): DependencyGraph {
        ...
        return DependencyGraph(dependencyIds, sortedSetOf(), scopeMapping, nodes, edges.removeAllCycles())
    }

    private fun Collection<DependencyGraphEdge>.removeAllCycles(): List<DependencyGraphEdge> {
        val edges = mapTo(mutableSetOf()) { it.from to it.to }
        val edgesToKeep = breakAllCycles(edges)
 ...
 */
internal fun breakAllCycles(edges: Collection<Pair<Int, Int>>): Set<Pair<Int, Int>> {
