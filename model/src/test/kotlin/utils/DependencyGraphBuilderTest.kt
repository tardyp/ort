...
class DependencyGraphBuilderTest : WordSpec({
    ...
    "breakAllCycles()" should {
        "not break undirected cycles" {
            ...
            breakAllCycles(edges) shouldContainExactlyInAnyOrder edges
        }

        "break a directed cycles with a single node" {
            val edges = listOf(1 to 1)

            breakAllCycles(edges) should beEmpty()
        }
        ...
        "break directed cycles involving multiple nodes" {
            ...
            val result = breakAllCycles(edges)
