...
/**
 * A class that contain all [ScanResult]s for a [NestedProvenance].
 */
data class NestedProvenanceScanResult(
    ...
    fun getProvenances(): Set<KnownProvenance> =
        setOf(nestedProvenance.root, *nestedProvenance.subRepositories.values.toTypedArray())

    /**
     * Return true if all [KnownProvenance]s contained in [nestedProvenance] are contained in [scanResults].
     */
    // TODO: Should this check if the scan results are not empty?
    fun isComplete(): Boolean = getProvenances().all { it in scanResults.keys }
}
